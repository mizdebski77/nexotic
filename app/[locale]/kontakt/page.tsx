...
// Each contact row pulls its label from translations; values (email, phone)
// stay constant. The location value is translated since "Polska" / "Poland".
const CONTACT_ITEMS = [
  {
    key: 'email',
  ...
  {
    key: 'location',
    value: null, // pulled from `kontaktPage.locationValue`
    ...
    <div className="bg-white pt-[68px]">
      <div className="bg-ink py-20 px-6 md:px-10 relative">
        <div className="max-w-site mx-auto ">
          ...
          <div className="text-[11px] font-bold tracking-[2px] uppercase text-lime mb-3">{t('heroEyebrow')}</div>
          ...
          <p className="text-base text-white/50 max-w-md leading-relaxed">{t('heroSubtitle')}</p>
        </div>
      </div>
      <div className="max-w-site mx-auto px-6 md:px-10 py-20 grid xl:grid-cols-[1fr_1.3fr] gap-20 items-start">
        <AnimatedSection direction="left">
          ...
          {CONTACT_ITEMS.map((item) => {
            const label =
              item.key === 'email' ? t('emailLabel')
              : item.key === 'phone' ? t('phoneLabel')
              : t('locationLabel')
            const value = item.value ?? t('locationValue')
            return (
              <div key={item.key} className="mb-6 pb-6 border-b border-neutral-100 last:border-0 flex items-start gap-4">
                <div className="w-10 h-10 min-w-10 rounded-xl bg-neutral-100 flex items-center justify-center">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#c8f135" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                    <path d={item.icon} />
                  </svg>
                </div>
                <div>
                  <div className="text-[11px] font-bold text-neutral-400 tracking-[1px] uppercase mb-1">{label}</div>
