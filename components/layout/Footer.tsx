  ...
  const cols = [
    {
      title: t('colServices'),
      links: [
        [tServices('web.title'), '/uslugi'],
        [tServices('mobile.title'), '/uslugi'],
        [tServices('ai.title'), '/uslugi'],
        [tServices('systems.title'), '/uslugi'],
        [tServices('chatbots.title'), '/uslugi'],
      ],
    },
    {
      title: t('colCompany'),
    ...
    },
    {
      title: t('colLegal'),
      links: [
        [t('privacy'), '/polityka-prywatnosci'],
        [t('terms'), '/regulamin'],
        [t('cookies'), '/cookies'],
      ],
    },
    ...
    <footer>
      ...
      <div className="bg-ink-2 border-t border-white/[0.07] py-16 px-6 md:px-10 relative overflow-hidden">
        ...
        <div className="max-w-site mx-auto flex flex-col md:flex-row items-start md:items-center justify-between gap-8 relative z-10">
          <div>
            <h2 className="text-3xl md:text-5xl font-extrabold tracking-[-1.5px] text-white leading-[1.06] mb-3">{t('ctaTitle')}</h2>
            <p className="text-sm text-white/40">{t('ctaSubtitle')}</p>
          </div>
        ...
        <div className="max-w-site mx-auto">
          <div className="grid grid-cols-2 md:grid-cols-[1.6fr_1fr_1fr_1fr] gap-10 mb-12">
            <div className="col-span-2 md:col-span-1">
              ...
              <p className="text-sm text-white/30 leading-relaxed max-w-[220px] mb-5">{t('tagline')}</p>
              ...
              <div key={title}>
                ...
                <ul className="flex flex-col gap-2.5 list-none">
                  {links.map(([label, href]) => (
                    <li key={label}>
                      <Link href={href as string} className="text-[13.5px] text-white/35 hover:text-white transition-colors duration-200 hover:translate-x-0.5 inline-block">
                        {label}
                      </Link>
                    </li>
                  ))}
          ...
          <div className="border-t border-white/[0.06] pt-5 text-center">
            <p className="text-[12.5px] text-white/20">{t('copyright')}</p>
          </div>
        </div>
      </div>
    </footer>
